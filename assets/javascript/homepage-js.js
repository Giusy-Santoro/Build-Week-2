...
function createArtistCard(artistData) {
  const { picture_big, name, title} = artistData;
  const card = document.createElement('div');
  card.className = 'card trend p-3 sm-6 md-6';
  card.style.width = '12rem';
  card.innerHTML = `
    <img src="${picture_big}" class="card-img-top" alt="${name}">
    <div class="card-body">
        <h6 class="d-block text-truncate" style="max-width: 150px">${name}</h6>
